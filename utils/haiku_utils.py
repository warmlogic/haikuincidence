...
def count_syllables(
    ...
    for subtoken in token_clean.split():
        ...
        if subtoken in syllable_dict:
            ...
            subsyllable_count += subtoken_syl
        elif (subtoken.endswith('s') or subtoken.endswith('z')) and (subtoken[:-1] in syllable_dict) and not text_might_contain_acronym(subtoken_orig):
            ...
            subsyllable_count += subtoken_syl
        elif (subtoken.endswith('s') or subtoken.endswith('z')) and (subtoken[:-1] in pronounce_dict) and not text_might_contain_acronym(subtoken_orig):
        ...
        for i, c in enumerate(word):
            ...
            if is_vowel:
                ...
                if not on_vowel:
                    ...
                    minsyl += 1
                    maxsyl += 1
                    logger.debug(f'new syllable: min syl {minsyl}, mean syl {(minsyl + maxsyl) // 2}, max syl {maxsyl}')
                    ...
                    in_diphthong = True
                    maxsyl += 1
                    logger.debug(f'diphthong: {c}: min syl {minsyl}, mean syl {(minsyl + maxsyl) // 2}, max syl {maxsyl}')
            else:
                if re.findall(r'[\w]', c):
